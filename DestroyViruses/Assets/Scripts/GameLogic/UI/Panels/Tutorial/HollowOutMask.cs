...
namespace DestroyViruses
{
    ...
    public class HollowOutMask : MaskableGraphic, ICanvasRaycastFilter
    {
        ...
        /// 设置镂空的目标
        /// </summary>
        public void SetTarget(RectTransform target,Action callback)
        {
            _canRefresh = true;
            _target = target;
            _callback = callback;
        ...
        bool ICanvasRaycastFilter.IsRaycastLocationValid(Vector2 screenPos, Camera eventCamera)
        {
            ...
            // 将目标对象范围内的事件镂空（使其穿过）
            bool result = !RectTransformUtility.RectangleContainsScreenPoint(_target, screenPos, eventCamera);
            //if (!result) _callback?.Invoke();
            return result;
        }
